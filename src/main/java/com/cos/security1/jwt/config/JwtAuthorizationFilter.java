...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter{
	...
	public JwtAuthorizationFilter(AuthenticationManager authenticationManager, UserRepository userRepository, ObjectMapper objectMapper, JwtRefreshTokenService jwtRefreshTokenService) {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		if (servletPath.equals("/login") || servletPath.equals("/refresh") || servletPath.equals("/join")) {
            chain.doFilter(request, response);
            return;
		}
		 ...
		 */
		try {
    		String jwtATToken = request.getHeader(JwtProperties.AT_HEADER_STRING).replace(JwtProperties.TOKEN_PREFIX, "");
			...
			if (accessTokenExpireTime > now) {
				DecodedJWT accessTokenVerify = JWT.require(Algorithm.HMAC512(JwtProperties.SECRET)).build().verify(jwtATToken);
				...
				String jwtRFToken = request.getHeader(JwtProperties.RT_HEADER_STRING).replace(JwtProperties.TOKEN_PREFIX, "");

				Map<String, String> verifyRefresh = jwtRefreshTokenService.refresh(jwtRFToken);
				System.out.println("verifyRefresh.get(JwtProperties.AT_HEADER_STRING) = " + verifyRefresh.get(JwtProperties.AT_HEADER_STRING));
				if ((!verifyRefresh.get(JwtProperties.AT_HEADER_STRING).isEmpty())){ // 2. acccess 토큰만 재발급
					...
					response.addHeader(JwtProperties.AT_HEADER_STRING, JwtProperties.TOKEN_PREFIX + newAccessToken);

					authunticateUser(userRepository.findByUserId(userId));
					chain.doFilter(request, response);

				} else if ((!verifyRefresh.get(JwtProperties.AT_HEADER_STRING).isEmpty()) && (!verifyRefresh.get(JwtProperties.RT_HEADER_STRING).isEmpty())) { // 3. access, refresh 토큰 둘다 재발급
					String newAccessToken = verifyRefresh.get(JwtProperties.AT_HEADER_STRING);
					...
					response.addHeader(JwtProperties.AT_HEADER_STRING, JwtProperties.TOKEN_PREFIX + newAccessToken);
					response.addHeader(JwtProperties.RT_HEADER_STRING, JwtProperties.TOKEN_PREFIX + newRefreshToken);
